...
def apply_action(node, action, market_states, stock_to_idx):
    ...
    returns = market_states[node.depth]["returns"]
...
def best_sequence(root):
    ...
    while len(node.children) > 0:
        node = max(node.children, key=lambda child: child.times_visited)
...
def print_sequence(root, market_states, stock_to_idx):
    ...
    for node in sequence:
        ...
        if action.stock is not None:
            ...
            return_1 = state["returns"][0, index].item()
            return_2 = state["returns"][1, index].item()
